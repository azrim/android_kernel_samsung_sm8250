...
/*
 * Mainline shares the radix tree node cache with the XArray (there,
 * radix_tree_node is an alias for xa_node).  This tree keeps the original
 * radix tree implementation, so XArray uses a private node cache of the
 * same design.  xa_node_rcu_free() mirrors radix_tree_node_rcu_free().
 */
...
static int __init xa_node_cache_init(void)
{
	xa_node_cachep = kmem_cache_create("xa_node", sizeof(struct xa_node), 0,
			SLAB_PANIC | SLAB_RECLAIM_ACCOUNT, xa_node_ctor);
	return 0;
}
subsys_initcall(xa_node_cache_init);
