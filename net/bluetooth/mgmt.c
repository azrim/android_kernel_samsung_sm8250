			      ...
			      bool mgmt_connected)
{
	...
	mgmt_pending_foreach(MGMT_OP_DISCONNECT, hdev, true, disconnect_rsp,
			     &sk);
...
void mgmt_ssp_enable_complete(struct hci_dev *hdev, u8 enable, u8 status)
{
	...
	if (status) {
		...
		mgmt_pending_foreach(MGMT_OP_SET_SSP, hdev, true, cmd_status_rsp,
				     &mgmt_err);
		return;
	}
				    ...
				    u8 status)
{
	...
	mgmt_pending_foreach(MGMT_OP_SET_DEV_CLASS, hdev, false, sk_lookup,
			     &match);
	mgmt_pending_foreach(MGMT_OP_ADD_UUID, hdev, false, sk_lookup, &match);
