...
got_pg:
	...
	if (stime_d / NSEC_PER_MSEC > 256) {
		...
		for_each_online_pgdat(p) {
			a_anon += node_page_state(p, NR_ACTIVE_ANON);
			in_anon += node_page_state(p, NR_INACTIVE_ANON);
			a_file += node_page_state(p, NR_ACTIVE_FILE);
			in_file += node_page_state(p, NR_INACTIVE_FILE);
		}
